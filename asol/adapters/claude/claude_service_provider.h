...
#ifndef ASOL_ADAPTERS_CLAUDE_CLAUDE_SERVICE_PROVIDER_H_
#define ASOL_ADAPTERS_CLAUDE_CLAUDE_SERVICE_PROVIDER_H_
...
// ClaudeServiceProvider implements the AIServiceProvider interface for Anthropic Claude.
class ClaudeServiceProvider : public core::AIServiceProvider {
 public:
  ClaudeServiceProvider();
  explicit ClaudeServiceProvider(const std::string& api_key);
  ~ClaudeServiceProvider() override;
 ...
 private:
  ...
  // Convert between ASOL and Claude message formats
  std::vector<ClaudeMessage> ConvertToClaudeMessages(
      const std::vector<core::ContextMessage>& context_messages);
  
  // Handle Claude adapter response
  void OnClaudeResponse(AIResponseCallback callback,
                      bool success,
                      const std::string& response);

  // The Claude adapter
  std::unique_ptr<ClaudeTextAdapter> claude_adapter_;
  
  // Configuration
  std::unordered_map<std::string, std::string> config_;
  
  // For weak pointers
  base::WeakPtrFactory<ClaudeServiceProvider> weak_ptr_factory_{this};
};
...
#endif  // ASOL_ADAPTERS_CLAUDE_CLAUDE_SERVICE_PROVIDER_H_